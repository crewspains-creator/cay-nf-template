...
def handle_callback(call):
    ...
    if data == "main_menu":
        edit_current_message(call, "🏠 <b>Main Menu</b>", main_menu_markup())
...
Use responsibly!"""
        ...
        for t, name in [("premium","👑 PREMIUM"), ("standard","⭐ STANDARD"), 
            ...
            status_text += f"{name}\nUsed: {used}/3\nStock: {STOCK.get(t, 0)}\n\n"
        status_text += "⏰ Limits reset every hour (rolling)."
        ...
        for t, name in [("premium","👑 PREMIUM"), ("standard","⭐ STANDARD"), 
                       ("basic","🎯 BASIC"), ("prime","🍿 PRIME VIDEO")]:
            stock_text += f"{name}: {STOCK[t]} accounts\n"
            stock_text += "▰▰▰▰▰▰▰▰▰▰\n\n"
        ...
        help_text = """ℹ️ <b>HOW TO USE DEADFLIX</b>

1. Choose a service & tier
2. Bot sends verified cookie
3. Import cookie into browser
4. Open the NFTToken link

Use cookies responsibly."""

        edit_current_message(call, help_text, main_menu_markup())

    elif data == "language":
        edit_current_message(call, "🌐 <b>Select your language:</b>", None)
        # You can expand this with language buttons later
